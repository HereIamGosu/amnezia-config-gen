...
- **Генерация конфигурации** для AmneziaWG с предустановленными маршрутами для Discord: режим **Legacy** (фиксированные магические заголовки H1=1..H4=4 и старые Jc/Jmin/Jmax — как в классическом профиле совместимости) или **AmneziaWG 2.0** (случайное **незапересекающееся** разбиение диапазона **5–2147483647** на четыре полосы H1–H4 — как в типичных инсталляторах AWG 2.0 и signed-int32‑совместимых клиентах; `amneziawg-go` по-прежнему парсит значения как uint32 и проверяет отсутствие пересечений; на каждом пакете выбирается случайное значение *внутри* своей полосы; дополнительно S1–S4 с ограничением `S1+56 ≠ S2`, Jc/Jmin/Jmax; нужен совместимый клиент, например AmneziaVPN 4.8.12.9+).
