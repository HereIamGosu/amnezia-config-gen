...
- **api/warp.js** — серверless-обработчик Vercel: регистрация WARP через `api.cloudflareclient.com`, ретраи, сборка `.conf` для Legacy или AWG 2.0; шаблоны **`template`**, запасной **`PublicKey`** пира, **`engage.cloudflareclient.com`** с портами 2408/4500; опционально `AllowedIPs` по `presets`, `I1`/`i1Ref`/встроенный пресет, `PersistentKeepalive`, `plainAddress`. **`api/warpAmneziaCpsPayload.js`** — эталонная строка I1 для `warp_amnezia`. Каталог **`api/cps-presets/`** — текстовые файлы для `i1Ref` (см. `README.txt` внутри).
